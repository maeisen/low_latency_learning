...
def mlp_model2(state_dim, action_dim, num_param, layers=[256, 128, 32]):
    with tf.variable_scope('policy'):
...
class ReinforcePolicy(object):
    def __init__(self,
        ...
        self._build_model(sys.state_dim, sys.action_dim, model_builder, distribution)
    ...
    def get_action(self, inputs):
        state_list, action_list = self.rollout(inputs,self.T)

        return state_list, action_list

    def rollout(self,inputs, T):
        
        batch_size = inputs.shape[0]
        all_actions = np.zeros((batch_size,self.action_dim,T))
        all_states = np.zeros((batch_size,self.state_dim,T))
        for i in np.arange(T):
            ...
            inputs = self.sys.update_system(inputs,action,batch_size,rate_select=True)
